...
void Table::ReadMeta(const Footer& footer) {
    if (rep_->options_paranoid_checks) {
        opt.verify_checksums = true;
    }
    BlockContents contens;
...
void Table::ReadFilter(const Slice& filter_handle_value) {
    ...
    // We might want to verify with ReadBlock() if we start
    // requring checksum verification in Table::Open.
    ReadOptions opt;
    ...
    rep_filter = new FilterBlockReader(rep_->options.filter_policy, block.data);
}
